...
function processarAB (txt) {
    ...
    function verificaSinal (str, i) {
        if (isNaN(str[i - 1]) == true && str[i] == '-' && isNaN(str[i + 1]) == false) {
        //"se anterior for op, se atual for '-' e proximo for numero  exemplo "(2+-1)"
            return true; //se representa o sinal de negativo
        } else {
            return false;
        }
    }
    ...
    function modoSimples () {
        for (var i = 0; i < string.length; i++) {   //percorrendo string
            if (isNaN(Number(string[i])) == false || string[i] == '.' || verificaSinal(string, i) == true) {   //compara se é número, ponto, ou se mem.op já existe então o '-' indica negativo
                if ((checkInternal(mem.op) == false && checkInternal(mem.r) == false)) {     //se primeiro operador não foi encontrado
                    if (string[i] == '.') {
                        ...
                        stringZ += string[i];
                        mem.a += stringZ;
                    } else if (verificaSinal(string, i) == true) {
                        mem.a += string[i];
                    } else {
                        mem.a += string[i];
                    }
    ...
    function modoAvancado () {
        for (var i = 0; i < string.length; i++) {   //percorrendo string
            if (isNaN(Number(string[i])) == false  || string[i] == '.' || verificaSinal(string, i) == true) {   //compara se é número
                if ((checkInternal(mem.op) == false && checkInternal(mem.r) == false)) {     //se primeiro operador não foi encontrado
                    if (string[i] == '.') {
                        let stringZ = '';
                        do {
    ...
    return Number(mem.r);       //Number(mem.r) pois quando 0 retorna ''
}

processarAB('2+-1*4/2+20*2/4=');
console.log('estado memoria:   a:' + mem.a + ' op: ' + mem.op + ' b: ' + mem.b + ' r: ' + mem.r);

module.exports = { processarAB };
